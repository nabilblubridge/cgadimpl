...
// ----- elementwise binary -----
void vjp_Add(Node* n, const std::shared_ptr<Node>& gy){
    Node* A = n->inputs[0].get(); Node* B = n->inputs[1].get();


    auto H = flomul_nodeops(gy, 1.0) ;
    if (A->requires_grad) A->grad.add_( rt(H->value, A->value) );
    if (B->requires_grad) B->grad.add_( rt(H->value, B->value) );
}
void vjp_Sub(Node* n, const std::shared_ptr<Node>& gy){
    Node* A = n->inputs[0].get(); Node* B = n->inputs[1].get();
      auto H = flomul_nodeops(gy, 1.0) ;
    if (A->requires_grad) A->grad.add_( rt(H->value, A->value) );
    if (B->requires_grad) B->grad.add_( rt(-(H->value), B->value) );
}
void vjp_Mul(Node* n, const std::shared_ptr<Node>& gy){
    Node* A = n->inputs[0].get(); Node* B = n->inputs[1].get();


    auto H = mul_nodeops(gy, A->shared_from_this()) ;
    if (A->requires_grad) A->grad.add_( rt(H->value, A->value) );
    if (B->requires_grad) B->grad.add_( rt(H->value, B->value) );
// }

// // ----- elementwise trinary -----
// void vjp_FMA(Node* n, const std::shared_ptr<Node>& gy){
...
//     // Shapes
//     const std::shared_ptr<Node>& At = A->value;
//     const std::shared_ptr<Node>& Bt = B->value;
//     auto [M, K]  = At.shape();
//     auto [K2, N] = Bt.shape();
...
// void vjp_LayerNorm(Node* n, const std::shared_ptr<Node>& gy){
...
// void vjp_RealLayerNorm(Node* n, const std::shared_ptr<Node>& gy){
...
// void vjp_RMSNorm(Node* n, const std::shared_ptr<Node>& gy){
...
// void vjp_RealRMSNorm(Node* n, const std::shared_ptr<Node>& gy){
...
// void vjp_Attention(Node* n, const std::shared_ptr<Node>& gy){
//     Node* A = n->inputs[0].get();
//     Node* B = n->inputs[1].get();
//     Node* C = n->inputs[2].get();
...
// void vjp_AlibiAttention(Node* n, const std::shared_ptr<Node>& gy){
//     Node* A = n->inputs[0].get();
//     Node* B = n->inputs[1].get();
//     Node* C = n->inputs[2].get();
//     Node* D = n->inputs[3].get();
...
// void vjp_SWIGLU(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X = n->inputs[0].get();
...
// // ----- unary activations -----
// void vjp_Relu(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X = n->inputs[0].get();
...
// void vjp_Tanh(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X = n->inputs[0].get();
//     if (!X->requires_grad) return;
...
// }
// void vjp_Sigmoid(Node* n, const std::shared_ptr<Node>& gy){
...
// }
// void vjp_Softplus(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X = n->inputs[0].get(); if (!X->requires_grad) return;
//     X->grad.add_( rt( gy * Tensor::sigmoid(X->value), X->value) );
// }

// void vjp_Gaus(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X = n->inputs[0].get(); if (!X->requires_grad) return;
//     X->grad.add_( rt( gy * -2*X->value*Tensor::exp(-1*X->value*X->value), X->value) );
// }

// void vjp_Transpose(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X = n->inputs[0].get(); if (!X->requires_grad) return;
...
// void vjp_SiLU(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X = n->inputs[0].get(); if (!X->requires_grad) return;
//     Tensor s = Tensor::sigmoid(X->value);
//     X->grad.add_( rt( gy * ( s + X->value * ( s * (Tensor::ones_like(s)-s) ) ), X->value) );
// }

// void vjp_Parcon(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X = n->inputs[0].get(); if (!X->requires_grad) return;
//     X->grad.add_( rt( gy * ( 2 *Tensor::ones_like(X->value)- 2*X->value  ), X->value) );
// }

// void vjp_LiSHT(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X = n->inputs[0].get(); if (!X->requires_grad) return;
...
// void vjp_GELU(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X = n->inputs[0].get(); if (!X->requires_grad) return;
...
// }
// void vjp_LeakyRelu(Node* n, const std::shared_ptr<Node>& gy){
...
// // ----- matmul -----
// void vjp_MatMul(Node* n, const std::shared_ptr<Node>& gy){
//    Node* A = n->inputs[0].get();
//     Node* B = n->inputs[1].get();
...
// void vjp_Dyntanh(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X = n->inputs[0].get(); 
//     Node* A = n->inputs[1].get(); 
//     Node* B = n->inputs[2].get(); 
...
// // ----- reductions -----
// void vjp_Sum(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X=n->inputs[0].get(); if(!X->requires_grad) return;
//     float s = gy(0,0);
//     X->grad.add_( Tensor::ones_like(X->value) * s );
// }
// void vjp_RowSum(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X=n->inputs[0].get(); if(!X->requires_grad) return;
//     // gy [B,1] broadcast across columns
//     Tensor g = gy * Tensor::ones_like(X->value);
//     X->grad.add_( g );
// }
// void vjp_RowMax(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X=n->inputs[0].get(); if(!X->requires_grad) return;
//     int R=X->value.rows(), C=X->value.cols();
...
//     X->grad.add_( g );
// }
// void vjp_MeanAll(Node* n, const std::shared_ptr<Node>& gy){
//     Node* X=n->inputs[0].get(); if(!X->requires_grad) return;
...
// // ----- softmax / losses -----
// void vjp_SoftmaxRow(Node* n, const std::shared_ptr<Node>& gy){
...
//     Z->grad.add_( g );
// }
// void vjp_LogSumExpRow(Node* n, const std::shared_ptr<Node>& gy){
//     Node* Z = n->inputs[0].get(); if(!Z->requires_grad) return;
//     Tensor y = Tensor::softmax_row(Z->value);
//     Z->grad.add_( y * gy ); // gy [B,1] broadcast
// }
// void vjp_CeWithLogits(Node* n, const std::shared_ptr<Node>& gy /*unused: scalar gy*/){
...
// void vjp_KLDivergence(Node* n, const std::shared_ptr<Node>& gy /*unused: scalar gy*/){
//     Node* Z = n->inputs[0].get();
//     Node* Y = n->inputs[1].get();
//     int B = Z->value.rows();
//     Tensor sm = Tensor::softmax_row(Z->value);
...
// void vjp_MSELoss(Node* n, const std::shared_ptr<Node>& gy /*unused: scalar gy*/){
//     Node* Z = n->inputs[0].get();
//     Node* Y = n->inputs[1].get();
...
// void vjp_MAELoss(Node* n, const std::shared_ptr<Node>& gy /*unused: scalar gy*/){
//     Node* Z = n->inputs[0].get();
//     Node* Y = n->inputs[1].get();
//     int B = Z->value.rows(), C = Z->value.cols();
...
void vjp_Leaf(Node*, const std::shared_ptr<Node>&){ /* no-op */ }
