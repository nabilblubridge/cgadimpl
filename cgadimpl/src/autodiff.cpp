...
Node* n = *it; if(!n->requires_grad) continue; 
...
case Op::Exp: {
Node* X=n->inputs[0].get(); if(X->requires_grad){ Tensor g = gy * Tensor::exp(X->value); X->grad.add_( Tensor::reduce_to(g, X->value) ); } break; }
case Op::Log: {
Node* X=n->inputs[0].get(); if(X->requires_grad){ Tensor g = gy / X->value; X->grad.add_( Tensor::reduce_to(g, X->value) ); } break; }
...
case Op::Sigmoid:{
Node* X=n->inputs[0].get(); if(X->requires_grad){ Tensor s = Tensor::sigmoid(X->value); Tensor g = gy * ( s * (Tensor::ones_like(s) - s) ); X->grad.add_( Tensor::reduce_to(g, X->value) ); } break; }
case Op::Softplus:{
Node* X=n->inputs[0].get(); if(X->requires_grad){ Tensor g = gy * Tensor::sigmoid(X->value); X->grad.add_( Tensor::reduce_to(g, X->value) ); } break; }
case Op::SiLU:{
Node* X=n->inputs[0].get(); if(X->requires_grad){ Tensor s = Tensor::sigmoid(X->value); Tensor g = gy * ( s + X->value * ( s * (Tensor::ones_like(s) - s) ) ); X->grad.add_( Tensor::reduce_to(g, X->value) ); } break; }
...
case Op::RowSum:{ Node* X=n->inputs[0].get(); if(X->requires_grad){ Tensor g = gy * Tensor::ones_like(X->value); X->grad.add_(g); } break; }
...
case Op::MeanAll:{ // *********
     Node* X=n->inputs[0].get(); if(X->requires_grad){ float s = gy(0,0)/(float)(X->value.rows() * X->value.cols()); Tensor g = Tensor::ones_like(X->value) * s; X->grad.add_(g); } break; }

case Op::SoftmaxRow:{ Node* Z=n->inputs[0].get(); if(Z->requires_grad){ Tensor y = n->value; Tensor dot = Tensor::row_sum( y * gy ); Tensor g = y * ( gy - dot ); Z->grad.add_(g); } break; }
...
case Op::CeWithLogits:{ Node* Z=n->inputs[0].get(); Node* Y=n->inputs[1].get(); int B = Z->value.rows();
...
case Op::Exp:{ Node* X=n->inputs[0].get(); t = T[X] * Tensor::exp(X->value); break; }
case Op::Log:{ Node* X=n->inputs[0].get(); t = T[X] / X->value; break; }
...
case Op::Sigmoid:{ Node* X=n->inputs[0].get(); Tensor s = Tensor::sigmoid(X->value); t = T[X] * ( s * (Tensor::ones_like(s) - s) ); break; }
case Op::Softplus:{ Node* X=n->inputs[0].get(); t = T[X] * Tensor::sigmoid(X->value); break; }
case Op::SiLU:{ Node* X=n->inputs[0].get(); Tensor s = Tensor::sigmoid(X->value); t = T[X] * ( s + X->value * ( s * (Tensor::ones_like(s) - s) ) ); break; }
...
case Op::SoftmaxRow:{ Node* Z=n->inputs[0].get(); Tensor y = n->value; Tensor dot = Tensor::row_sum( y * T[Z] ); t = y * ( T[Z] - dot ); break; }
...
case Op::CeWithLogits:{ Node* Z=n->inputs[0].get(); Node* Y=n->inputs[1].get(); int B=Z->value.rows(); Tensor sm = Tensor::softmax_row(Z->value); Tensor gZ = (sm - Y->value) * (1.0f/(float)B); Tensor tZ = T.count(Z)? T[Z] : Tensor::zeros_like(Z->value); Tensor tY = T.count(Y)? T[Y] : Tensor::zeros_like(Y->value); Tensor lse = Tensor::logsumexp_row(Z->value); Tensor lsm = Z->value - lse; Tensor gY = (lsm * (-1.0f/(float)B)); Tensor dotZ(1,1); dotZ(0,0) = (gZ * tZ).sum_scalar(); Tensor dotY(1,1); dotY(0,0) = (gY * tY).sum_scalar(); t = dotZ + dotY; break; }
}
T[n] = t;
}
